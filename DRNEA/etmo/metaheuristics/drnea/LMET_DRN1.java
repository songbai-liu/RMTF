...
public class LMET_DRN1 extends Algorithm {
	int populationSize;
	int maxEvaluations;
	int evaluations;
	SolutionSet[] archive;
	 ...
	 *            Problem to solve
	 */
	public LMET_DRN1(ProblemSet problemSet) {
		super(problemSet);
	...
	public void initialization() throws JMException, ClassNotFoundException {
		...
		learningRate = 0.1;
		for(int t=0;t<noTasks;t++) {
			DAE[t] = new DenoiseAutoEncoder(numVars[t], numCodes, 5, learningRate);
		}
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {		
		initialization();
		// Generations
		while (evaluations < 1*maxEvaluations) {
			reproduction1(0.1);
			getNextPopulation();
		} // while
		getTraningModels();
		while (evaluations < 0.35*maxEvaluations) {
			reproduction5(0.5);
			getNextPopulation();
		}
	...
	public void getTraningModels() throws JMException {
		...
		for(int p=0;p<populationSize;p++) {
			...
			DAE[sf].trainingModel(sol, epochs);
			double[] encodes = DAE[sf].encode(sol,numVars[sf]);
	...
	public void reproduction3(double rmp) throws JMException, ClassNotFoundException {
		Solution[] parents = new Solution[2];
		for (int i = 0; i < (populationSize / 2); i++) {
			parents[0] = (Solution) selectionOperator.execute(conPopulation);
			parents[1] = (Solution) selectionOperator.execute(conPopulation);			
			...
			if (sfs[0] == sfs[1] || rand < rmp) {
				...
				double[][] encoder = new double[2][];
				//Encode via the related DAE models
				encoder[0] = encodeWithDAE(offSpring[0],sfs[0]);
				encoder[1] = encodeWithDAE(offSpring[1],sfs[1]);
				for(int c=0;c<2;c++){
					rd = PseudoRandom.randDouble();
					double[] tar = DIS.computeTarget(encoder[c]);
					int pT = 0;
					double maxV = tar[0];
					for(int tt=1;tt<noTasks;tt++){
						if(maxV < tar[tt]){
					...
					if(pT != sfs[c]) {
						if(rd < maxV){
							//Decode via the related DAE models
							offSpring[c] = decodeWithDAE(encoder[c],pT);
						}else {
							pT = sfs[c];
						}
					}
	...
	public void reproduction4(double rmp) throws JMException, ClassNotFoundException {
		Solution[] parents = new Solution[2];
		for (int i = 0; i < (populationSize / 2); i++) {
			parents[0] = (Solution) selectionOperator.execute(conPopulation);
			parents[1] = (Solution) selectionOperator.execute(conPopulation);			
