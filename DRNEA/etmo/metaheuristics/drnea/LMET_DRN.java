...
public class LMET_DRN extends Algorithm {
	int populationSize;
	int maxEvaluations;
	int evaluations;
	SolutionSet[] archive;
	 ...
	 *            Problem to solve
	 */
	public LMET_DRN(ProblemSet problemSet) {
		super(problemSet);
	...
	public void initialization() throws JMException, ClassNotFoundException {
		...
		DAE = new DenoiseAutoEncoder(features, numCodes, 3, learningRate);
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {		
		initialization();
		// Generations
		while (evaluations < 1.5*maxEvaluations) {
			reproduction1(0.1);
			getNextPopulation();
		} // while
		getTraningModels();
	...
	public void getTraningModels() throws JMException {
		...
		int features = problemSet_.getMaxDimension();
		for(int t=0;t<noTasks;t++) {
			for(int p=0;p<archive[t].size();p++) {
				Solution sol = archive[t].get(p);
				int sf = t;
				...
				DAE.baseAE.trainDecoder(inputs);
				double[] encodes = DAE.encode(sol,features);
				double[] targets = new double[noTasks];
				for(int tt=0; tt<noTasks; tt++){
					if(tt == sf) targets[tt] = 1.0;
					else targets[tt] = 0.0;
				}
				DIS.trainingModel(encodes, targets);
				double[] cError = DIS.classifier.getError();
				double[] dError = DAE.baseAE.getDecoderError();
				double[] error = new double[numCodes];
				for(int c=0;c<numCodes;c++) {
					error[c] = dError[c] - cError[c];
				}
				DAE.baseAE.trainEncoder(error);
			}
		}
		
		for(int p=0;p<populationSize;p++) {
			Solution sol = conPopulation.get(p);
			int sf = sol.getSkillFactor();
			double[] inputs = new double[features]; 
			XReal xSol = new XReal(sol);
			for(int j=0;j<features;j++) {
				inputs[j] = xSol.getValue(j);
	...
	public void tranning(Solution sol) throws JMException {
		int features = problemSet_.getMaxDimension();
		int sf = sol.getSkillFactor();
		double[] inputs = new double[features]; 
		XReal xSol = new XReal(sol);
	...
	public void reproduction1(double rmp) throws JMException, ClassNotFoundException {
		...
		for (int i = 0; i < (populationSize / 2); i++) {
			...
			if (sfs[0] == sfs[1] || rand < rmp) {
				...
				archive[sfs[p0]].add(offSpring[0]);
				archive[sfs[p1]].add(offSpring[1]);
			} else {
	...
	public void reproduction5(double rmp) throws JMException, ClassNotFoundException {
		Solution[] parents = new Solution[2];
		for (int i = 0; i < (populationSize / 2); i++) {
			parents[0] = (Solution) selectionOperator.execute(conPopulation);
			parents[1] = (Solution) selectionOperator.execute(conPopulation);			
			...
			if (sfs[0] != sfs[1] && rand < rmp) {
				...
				encoder[0] = encodeWithDAE(parents[0]);
				encoder[1] = encodeWithDAE(parents[1]);
				...
				if(PseudoRandom.randDouble() < 0.8) {
					...
					for(int c=0;c<2;c++){
						...
						encoder[2] = encodeWithDAE(parents[c]);
						int size = archive[sfs[c]].size();
						if(size > 3) {
							Solution[] exParents = new Solution[3];
							exParents[2] = parents[c];
							...
							exParents[0] = archive[sfs[c]].get(rdInt1);
							exParents[1] = archive[sfs[c]].get(rdInt2);
							encoder[0] = encodeWithDAE(exParents[0]);
							encoder[1] = encodeWithDAE(exParents[1]);
							childCode[c] = doCrossover1(encoder);
						}else {
							childCode[c] = encodeWithDAE(parents[c]);
						}
					}
					for(int c=0;c<2;c++){
	...
	public double[][] encodeWithDAE(Solution[] parents) throws JMException {
		...
		for(int i=0;i<parents.length;i++) {
			...
			encodedParents[i] = DAE.encode(parents[i],problemSet_.getMaxDimension());
		}
		return encodedParents;
	}
	
	public double[] encodeWithDAE(Solution parent) throws JMException {
		double[] encodedParents;
		XReal xsol = new XReal(parent);
		
		encodedParents = DAE.encode(parent, problemSet_.getMaxDimension());
	...
	public Solution decodeWithDAE(double[] encoder, int task) throws JMException, ClassNotFoundException {
		...
		double[] newDecode = DAE.decode(encoder);
		double value;
