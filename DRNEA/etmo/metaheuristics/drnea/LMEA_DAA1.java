...
public class LMEA_DAA1 extends Algorithm {
	int populationSize;
	int maxEvaluations;
	int evaluations;
	SolutionSet[] archive;
	 ...
	 *            Problem to solve
	 */
	public LMEA_DAA1(ProblemSet problemSet) {
		super(problemSet);
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {		
		...
		/*SolutionSet U = archive[0];
		for(int i=1;i<noTasks;i++) {
			U = ((SolutionSet) U).union(archive[i]);
		}
		return U;*/
	...
	public void reproduction() throws JMException, ClassNotFoundException {
		...
		for (int i = 0; i < (populationSize / 2); i++) {
			...
			Solution[] offSpring;
			if(evaluations < 1.1*maxEvaluations) {
				if (sfs[0] == sfs[1] || rand < 0.1) {
				...
				if (sfs[0] == sfs[1] || rand < 0.5) {
					...
					for(int c=0;c<2;c++){
						...
						offSpring[c] = decodeWithDAE(newEncoder[c],target);
						resetObjectives(offSpring[c]);
						//Evaluation
						problemSet_.get(target).evaluate(offSpring[c]);
						problemSet_.get(target).evaluateConstraints(offSpring[c]);
						archive[target].add(offSpring[c]);
					}
				}else {
					...
					for(int c=0;c<2;c++){
						offSpring[c] = decodeWithDAE(childCode[c],sfs[c]);
						problemSet_.get(sfs[c]).evaluate(offSpring[c]);
						problemSet_.get(sfs[c]).evaluateConstraints(offSpring[c]);
						archive[sfs[c]].add(offSpring[c]);
					}
