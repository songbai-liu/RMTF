 ...
 */
public class DRNEA_New1 extends Algorithm {
	int populationSize;
	int maxEvaluations;
	int evaluations;
	 ...
	 *            Problem to solve
	 */
	public DRNEA_New1(ProblemSet problemSet){
		super(problemSet);
			...
			for (int i = 0; i < (populationSize/noTasks/2); i++) {
				...
				if (rnd < 0.25) {
					...
					if(size > 3) {
						...
						XReal[] xeParents = new XReal[3];
						xeParents[0] = new XReal(parents[0]);
						xeParents[1] = new XReal(exParents[0]);
						xeParents[2] = new XReal(exParents[1]);
						double[] encode = encode(xeParents, t);
						double newEncode = encode[0] + 0.5*(encode[1] - encode[2]);
						if(newEncode < 0) { newEncode = 0.00001; } 
						if(newEncode > 1) { newEncode = 0.99999; }
						decode(newEncode,offSpring,t);
					}
					mutationOperator.execute(offSpring);
					offSpring.setSkillFactor(t);
					problemSet_.get(t).evaluate(offSpring);
					problemSet_.get(t).evaluateConstraints(offSpring);
					offspringPopulation[t].add(offSpring);
					archive[t].add(offSpring);
					evaluations += 1;
				}// if
			} // for			
		}	
	...
	public double[] encode(XReal[] parents, int t) throws JMException {
		int len = parents.length;
		double[] encodedParents = new double[len];
		for(int l=0;l<len;l++) {
			encodedParents[l] = 0;
			for(int d=0;d<numVars[t];d++) {
				encodedParents[l] += parents[l].getValue(d);
			}
			encodedParents[l] = encodedParents[l]/numVars[t];
		}
		return encodedParents; 
	}
	
	public void decode(double newEncode, Solution child, int t) throws JMException {
		XReal offspring = new XReal(child);
		double sum = 0;
		for(int var=0;var<numVars[t];var++) {
			sum += offspring.getValue(var);
		}
		for(int var=0;var<numVars[t];var++) {
			double decodedValue = (offspring.getValue(var)/sum) * newEncode * numVars[t];
			if(decodedValue <= 0.0) {
				decodedValue = 0.0 + 0.00001;
			}
			if(decodedValue >= 1.0) {
				decodedValue = 1.0 - 0.00001;
			}
			offspring.setValue(var, decodedValue);
		}
	}
