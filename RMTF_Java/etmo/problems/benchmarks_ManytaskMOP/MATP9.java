...
public class MATP9 {
	...
	public static ProblemSet getT(int taskID) throws IOException {
		...
		double[][] matrix = IO.readMatrixFromFile("MData/ManyTask/benchmark_9/matrix_"+(taskID+1));
		
		double shiftValues[] = IO.readShiftValuesFromFile("MData/ManyTask/benchmark_9/bias_"+(taskID+1));
		
		prob.setRotationMatrix(matrix);
		prob.setShiftValues(shiftValues);		
		
		((Problem)prob).setName("MATP9_"+(taskID+1));
