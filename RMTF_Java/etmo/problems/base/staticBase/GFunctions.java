...
public class GFunctions {
	...
	/*
	 ********************* GFunctions for CEC2017 and CEC2018 Benchmark Problems ***********************
	*/
	/*F171: Sphere Function*/	//Used in CIHS, PIHS, PIMS, NIHS
	public static double getF171(double x[], double[] shiftValues, double[][] rotationMatrix) {
	...
	/*F172: Mean Function*/	   //Used in CIHS, CIMS
	public static double getF172(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
		...
		for (int i = 0; i < x.length; i++)
			sum += Math.abs(x[i]);		
		return (sum * 9) / x.length;
	}
	
	/*F173: Rosenbrock Function*/ //Used in CIMS, NIHS, NIMS
	public static double getF173(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
	...
	/*F174: Rastrigin Function*/  //Used in CILS, PIHS, PIMS
	public static double getF174(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
		return BaseFunctions.getRastrigin(x);
	}	
	
	/*F175: Ackley Function*/	//Used in CILS, PILS, NILS
	public static double getF175(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
		return BaseFunctions.getAckley(x);
	}	
	
	/*F176: Griewank Function*/	//Used in PILS, NILS
	public static double getF176(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
		return BaseFunctions.getGriewank(x);
	}	
	/*
	 ************************************************ End **********************************************
	*/
	...
	/*
	 *************************** GFunctions for CEC2019 Benchmark Problems *****************************
	*/
	/*F191: GFunction Used in CPLX1-1, CPLX2-1*/
	public static double[] getF191(double[] xI, double xIII[][], int[][] index, int D) {
	...
	/*F192: GFunction Used in CPLX1-2, CPLX3-1, CPLX4-1, CPLX4-2, CPLX6-2, CPLX9-2*/
	public static double[] getF192(double[] xI, double xIII[][], int[][] index, int D) {
		int numObj = xIII.length;
		double[] g = new double[numObj];
		double linkage;
		for(int i=0; i<numObj; i++){
			g[i] = 0.0;
			for(int j=0; j<xIII[i].length; j++){
				linkage = xIII[i][j] - Math.sin(6*Math.PI*xI[0] + (index[i][j]*Math.PI)/D);
				g[i] += Math.pow(linkage, 2);
			}
			g[i] = 2*g[i]/xIII[i].length;
		}
	...
	/*F193: GFunction Used in CPLX2-2, CPLX8-2, CPLX10-1*/
	public static double[] getF193(double[] xI, double xIII[][], int[][] index, int D) {
		int numObj = xIII.length;
		double[] g = new double[numObj];
		double linkage;
		for(int i=0; i<numObj; i++){
			...
			for(int j=0; j<xIII[i].length; j++){
				linkage = xIII[i][j] - Math.pow(xI[0],0.5*(1.0+(3.0*(index[i][j]-2)/(D-2))));
				g[i] += 1.0 + 4*linkage*linkage - Math.cos(8*linkage*Math.PI);
			}
			g[i] = 2*g[i]/xIII[i].length;
		}
		return g;
	...
	/*F194: GFunction Used in CPLX5-1, CPLX6-1*/
	public static double[] getF194(double[] xI, double xIII[][], int[][] index, int D) {
		int numObj = xIII.length;
		double[] g = new double[numObj];
		double linkages;
		for(int i=0; i<numObj; i++){
	...
	/*F195: GFunction Used in CPLX5-2*/
	public static double[] getF195(double[] xI, double xIII[][], int[][] index, int D) {
		int numObj = xIII.length;
		double[] g = new double[numObj];
		double linkages;
		for(int i=0; i<numObj; i++){
	...
	/*F196: GFunction Used in CPLX3-2, CPLX7-1*/
	public static double[] getF196(double[] xI, double xIII[][], int[][] index, int D) {
		int numObj = xIII.length;
		double[] g = new double[numObj];
		double linkages;
		for(int i=0; i<numObj; i++){
			...
			for(int j=0; j<xIII[i].length; j++){
				if(i % 2 == 0){
					linkages = 0.8*xI[0]*Math.cos((6*Math.PI*xI[0] + (index[i][j]*Math.PI)/D)/3.0);
				}else {
					linkages = 0.8*xI[0]*Math.sin(6*Math.PI*xI[0] + (index[i][j]*Math.PI)/D);
				}
	...
	/*F197: GFunction Used in CPLX7-2, CPLX8-1, CPLX9-1*/
	public static double[] getF197(double[] xI, double xIII[][], int[][] index, int D) {
		int numObj = xIII.length;
		double[] g = new double[numObj];
		double linkages;
		for(int i=0; i<numObj; i++){
			...
			for(int j=0; j<xIII[i].length; j++){
				if(i % 2 == 0){
					linkages = (0.3*xI[0]*xI[0]*Math.cos(24*Math.PI*xI[0] + 4*(index[i][j]*Math.PI)/D) + 0.6*xI[0])
							*Math.cos(6*Math.PI*xI[0] + (index[i][j]*Math.PI)/D);
				}else {
					linkages = (0.3*xI[0]*xI[0]*Math.cos(24*Math.PI*xI[0] + 4*(index[i][j]*Math.PI)/D) + 0.6*xI[0])
	...
	/*F198: GFunction Used in CPLX10-1*/
	public static double[] getF198(double[] xI, double xIII[][], int[][] index, int D) {
		int numObj = xIII.length;
		double[] g = new double[numObj];
		double linkage;
		double part1, part2;
	...
	/*F199: GFunction Used in*/
	public static double[] getF199(double[] xI, double xIII[][], int[][] index, int D) {
		int numObj = xIII.length;
		double[] g = new double[numObj];
		for(int i=0; i<numObj; i++){
			g[i] = 0.0;
			for(int j=0; j<xIII[i].length; j++){
				g[i] += ((0.5*xIII[i][j]+0.25)-0.5)*((0.5*xIII[i][j]+0.25)-0.5);
			}
		}
		return g;
	}
	
	/*F200: GFunction Used in*/
	public static double[] getF200(double[] xI, double xIII[][], int[][] index, int D) {
		int numObj = xIII.length;
		double[] g = new double[numObj];
		for(int i=0; i<numObj; i++){
			g[i] = 0.0;
	...
	*/
	...
	/*
	 *************************** GFunctions for Many-Task Problems *****************************
	*/
	/*FM1: Sphere Function */
	public static double getFM1(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
	...
	/*FM2: Rastrigin Function */
	public static double getFM2(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
	...
	/*FM3: Griewank Function */
	public static double getFM3(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
	...
	/*FM4: Rosenbrock Function */
	public static double getFM4(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
	...
	/*FM5: Ackley Function */
	public static double getFM5(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
	...
	/*FM6: Weierstrass Function */
	public static double getFM6(double x[], double[] shiftValues, double[][] rotationMatrix) {
		shiftValues_ = shiftValues;
		rotationMatrix_ = rotationMatrix;
		shiftVariables(x);
		x=rotateVariables(x);
		return BaseFunctions.getWeierstrass(x);
	}
