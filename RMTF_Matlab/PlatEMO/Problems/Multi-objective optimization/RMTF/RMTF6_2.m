classdef RMTF6_2 < PROBLEM
% <multi/many> <real> <large/none>
% The neural network training problem
% nHidden1 --- 12 --- Size of the first hidden layer
% nHidden2 --- 6 --- Size of the second hidden layer
...
% Statlog_German                     1000      24       2
    ...
    methods
        ...
        function Setting(obj)
            [obj.nHidden1,obj.nHidden2] = obj.ParameterSet(12,6);
			% Load data
            Data  = load('Dataset\DryBeanCaliHoroz.txt');
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for i = 1 : size(PopDec,1)
				...
				Z = Predict(obj.TrainIn,W1,W2,W3);%Tanh activation
                %L2 regularization term
				PopObj(i,1) = sqrt(mean(PopDec(i,:).^2));
				%Mean Squared Error
                PopObj(i,2) =  mean((Z-obj.TrainLabel).^2);
            end
        end
...
function [Z,Y1,Y2] = Predict(X,W1,W2,W3)
    Y1 = 1 - 2./(1+exp(2*[ones(size(X,1),1),X]*W1));%Tanh
	Y2 = 1 - 2./(1+exp(2*[ones(size(Y1,1),1),Y1]*W2));%Tanh
