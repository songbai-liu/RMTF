...
% Statlog_Australian                  690      14       2
    ...
    methods
        ...
        function Setting(obj)
			...
			% Load data
            Data  = load('Dataset\DryBeanBarbunyaSira.txt');
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for i = 1 : size(PopDec,1)
                ...
                Z  = Predict(obj.TrainIn,W1,W2,W3);%Sigmoid
...
function [Z,Y1,Y2] = Predict(X,W1,W2,W3)
    Y1 = 1./(1+exp(-[ones(size(X,1),1),X]*W1));%Sigmoid
    Y2 = 1./(1+exp(-[ones(size(Y1,1),1),Y1]*W2));%Sigmoid
    Z = 1./(1+exp(-[ones(size(Y2,1),1),Y2]*W3));%SoftSign
end